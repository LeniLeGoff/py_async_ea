...
def load_learning_data_agg(folder):
    ...
    iter = 0
    prev_idx = []
    for lt_l, ld_l,idx_l, n_idx_l, t, ct, dc in zip(lt_lines,ld_lines,idx_lines,new_idx_lines,time_data,cum_time_data, disc_time_data):
...
def load_morph_norms_agg(folder):
    ...
    prev_idx = []
    new_ind = 0
    for mn_l,idx_l,n_idx_l, t,ct,dt in zip(mn_lines,idx_lines, new_idx_lines, time_data,cum_time_data,disc_time_data):
